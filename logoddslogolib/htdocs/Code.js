  ...
  window.ao.getinput = function( e, c ) {
   var a, i;
    ...
    if( !!(e = _( e )) ) {
      ...
      for( i=0; i<a.length; i++ ) if( a[i].name != "" ) {
        c = c.replace( RegExp( "(^|#)" + a[i].name + "#[^#]+", "g" ), "" );
        if( a[i].value != a[i].defaultValue ) c += "#" + a[i].name + "#" +  encodeURIComponent( a[i].value );
      }
    }
    return c.replace( /^#/, "" );
  };
    ...
    if( !!(e = _( e )) && !!s ) {
      if( !!(a = s.split( "#" )) ) for( i=1; i<a.length; i+=2 ) {
        ...
        if( s ) for( k=0; k<s.length; k++ ) if( !!(e = s[k]) ) {
          ...
          else                    try { e.value = decodeURIComponent( a[i] ); } catch( e ) { ; };
        }
      }
    }
  };
...
//Print Obj
  ...
  if( !!window.onload ) ao.addOnLoad( window.onload );
  ao.addOnLoad( _cat );
